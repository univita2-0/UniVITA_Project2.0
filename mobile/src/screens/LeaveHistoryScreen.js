...
import { View, Text, StyleSheet, SafeAreaView, FlatList, TouchableOpacity, ActivityIndicator, StatusBar, Modal, RefreshControl } from 'react-native';
import { useSafeAreaInsets } from 'react-native-safe-area-context';
import { Calendar as RNCalendar } from 'react-native-calendars';
import { ArrowLeft, Calendar as CalendarIcon, FileText, AlertCircle, ChevronLeft, ChevronRight, X, Eye, EyeOff, Clock, CheckCircle, XCircle } from 'lucide-react-native';
...
const formatDate = (dateStr) => {
  ...
  return isNaN(d.getTime()) ? cleanStr : d.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' });
};
...
export default function LeaveHistoryScreen({ navigation }) {
  ...
  // Groups consecutive days into a single card
  const groupConsecutiveLeaves = (leaves) => {
    if (!leaves || leaves.length === 0) return [];
    
    // Sort oldest to newest to detect consecutive dates accurately
