...
import { ArrowLeft, Calendar as CalendarIcon, Clock, MapPin, BookOpen, ChevronLeft, ChevronRight, X, Eye, EyeOff } from 'lucide-react-native';
...
export default function ScheduleHistoryScreen({ navigation }) {
  ...
  const handleCancelRequest = async () => {
    ...
    try {
      const token = await AsyncStorage.getItem('auth_token');
      const res = await fetch(`${API_URL}/schedule-requests/${selectedRequestId}/cancel`, {
        method: 'PUT',
        headers: {
          ...
          renderItem={({ item }) => {
            ...
            return (
              <View style={styles.historyCard}>
                ...
                {item.course && (
                  <View style={styles.cardBody}>
                    <BookOpen size={16} color={isLight ? "#64748B" : colors.textSecondary} />
                ...
                {item.place && (
                  <View style={styles.cardBody}>
                    <MapPin size={16} color={isLight ? "#64748B" : colors.textSecondary} />
...
const getDynamicStyles = (colors, isLight) => StyleSheet.create({
  ...
  detailText: { fontFamily: 'Inter_18pt-Medium', fontSize: 14, color: isLight ? '#475569' : colors.textSecondary },
