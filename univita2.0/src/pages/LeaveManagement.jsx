          ...
          <>
            <div className="expert-table-wrapper">
              <table className="expert-table">
                <thead>
                  <tr>
                    <th className="nowrap">Employee Details</th>
                    <th className="nowrap">Leave Type</th>
                    <th className="nowrap">Date Range</th>
                    <th className="text-center nowrap">Duration</th>
                    <th>Reason</th>
                    {view === 'pending' ? <th className="text-right nowrap">Actions</th> : <th className="text-center nowrap">Status</th>}
                  </tr>
                </thead>
                <tbody>
                  {currentGroups.map(group => (
                    <tr key={group.ids?.[0] || Math.random()}>
                      ...
                      <td className="font-medium text-dark nowrap">{formatDateRange(group.start_date, group.end_date)}</td>
                      <td className="text-center font-semibold text-muted nowrap">{group.request_count || 1} day(s)</td>
                      <td className="max-w-xs truncate wrap-text" title={group.reason}>
                        {group.reason || '—'}
                      </td>
                      {view === 'pending' ? (
                        <td className="nowrap">
                        ...
                        <td className="text-center nowrap">
                          <span className={`expert-chip ${group.status?.toLowerCase() === 'approved' ? 'success' : group.status?.toLowerCase() === 'rejected' ? 'danger' : 'warning'}`}>
                            {group.status?.toLowerCase() === 'approved' && <Check size={14} style={{ marginRight: '4px' }} />}
                            {group.status?.toLowerCase() === 'rejected' && <X size={14} style={{ marginRight: '4px' }} />}
